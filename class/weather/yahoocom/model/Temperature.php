...
class Temperature {
    ...
    public function __construct($degree, $unit) {
        ...
        case 'C':   $this->degreeCelsius = (double)$degree; break;
        case 'F':   $this->degreeCelsius = TemperatureConverter::FtoC((double)$degree); break;
        default:    throw new Exception('Unknown temperature unit was given');
        }
    }
     ...
     * マジックメソッド __toString()
     *
     * @return string セルシウス度に単に「℃」をつけたものを返す。精度は適当。getAsCelsius() で取得してフォーマットして使用することを推奨
     */
    ...
    public function getAsFahrenheit() {
        return TemperatureConverter::CtoF($this->degreeCelsius);
    }
}
